...
export default function AuthPage() {
  ...
  const toggleAuthMode = () => {
    ...
    setLocalError('');
    clearError();
    setFormData({ fullName: '', email: '', password: '', confirmPassword: '', agreedToTerms: false });
  };
  ...
  // Google OAuth flow: get access token → fetch profile → set user in store
  ...
  return (
    <div
      ...
      style={{
        ...
        backgroundImage:
          'radial-gradient(at 0% 0%, rgba(31, 104, 249, 0.15) 0px, transparent 50%), radial-gradient(at 100% 100%, rgba(31, 104, 249, 0.1) 0px, transparent 50%)',
      }}
    >
      {/* Top Navigation */}
            ...
            <div key={isLogin ? 'login' : 'signup'} className="animate-in fade-in slide-in-from-bottom-4 duration-500 ease-out fill-mode-both">
              ...
              <form className="space-y-4" onSubmit={handleSubmit}>
                ...
                {!isLogin && (
                  <div className="flex items-center mt-4">
                    ...
                    <label htmlFor="terms" className="ml-2 block text-xs text-slate-400">
                      I agree to the{' '}
                      <Link href="#" className="text-[#1f68f9] hover:underline">Terms of Service</Link> and{' '}
                      <Link href="#" className="text-[#1f68f9] hover:underline">Privacy Policy</Link>
                    </label>
                  </div>
                )}
                ...
                <div className="grid grid-cols-2 gap-4">
                  {/* ✅ LIVE Google Login Button */}
                  <button
                    type="button"
                    onClick={() => handleGoogleLogin()}
                    disabled={googleLoading}
                    className="flex items-center justify-center gap-2 rounded-lg border border-slate-700 bg-slate-800/50 py-2.5 px-4 text-sm font-semibold text-slate-200 hover:bg-slate-800 transition-all disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {googleLoading ? (
                  ...
                  {/* <button
                    type="button"
                    className="flex items-center justify-center gap-2 rounded-lg border border-slate-700 bg-slate-800/50 py-2.5 px-4 text-sm font-semibold text-slate-200 hover:bg-slate-800 transition-all"
                  >
